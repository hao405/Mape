...
__global__ void forward_inference_kernel(int T, int H,
    F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, bf *y_, float *s_, float *h0_) {
    int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
    float state[C] = {0};
    __shared__ float q[C], k[C], w[C], a[C], b[C];
    int64_t h0_base = ((int64_t)bb * H + hh) * C * C + i * C;
...
__global__ void forward_kernel_with_mask(int T, int H,
    F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_,
    bf* y_, float* s_, float* sa_, float* h0_, const bf* __restrict__ mask_) {
    int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
    float state[C] = {0};
    __shared__ float q[C], k[C], w[C], a[C], b[C];
...
__global__ void backward_kernel_with_mask(int T, int H,
    F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, const bf* __restrict__ mask_, F_ dy_,
    ...
    for (int t = T-1; t >= 0; t--) {
        ...
        for (int j = 0; j < C; j++) {
            ...
            float trans_col = dstateT_param[j] * iwi + ai * dSb_shared[j];
...
__global__ void forward_inference_kernel_with_mask(int T, int H,
    F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, bf *y_, float *s_, float *h0_, const bf* __restrict__ mask_) {
    int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
    float state[C] = {0};
    __shared__ float q[C], k[C], w[C], a[C], b[C];
    int64_t h0_base = ((int64_t)bb * H + hh) * C * C + i * C;
...
void cuda_forward(int B, int T, int H, bf*w, bf*q, bf*k, bf*v, bf*a, bf*b, bf*y, float*s, float*sa, float* h0) {
    forward_kernel<_C_><<<dim3(H,B), dim3(_C_)>>>(T,H,w,q,k,v,a,b,y,s,sa,h0);
}
...
void cuda_backward(int B, int T, int H,
    ...
    backward_kernel<_C_><<<dim3(H,B), dim3(_C_)>>>(T,H,w,q,k,v,a,b,dy,s,sa,dht,dh0,dw,dq,dk,dv,da,db);
}
...
void cuda_backward_with_mask(int B, int T, int H,
    bf* w, bf* q, bf* k, bf* v, bf* a, bf* b, bf* mask, bf* dy,
    float* s, float* sa, float* dht, float* dh0,
    bf* dw, bf* dq, bf* dk, bf* dv, bf* da, bf* db) {
    assert(T%_CHUNK_LEN_ == 0);
