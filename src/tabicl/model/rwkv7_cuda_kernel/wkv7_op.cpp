...
#ifndef TABICL_RWKV7_OP_NAMESPACE
#define TABICL_RWKV7_OP_NAMESPACE tabicl_wind_backstepping
#endif
...
void cuda_backward_with_mask(int B, int T, int H, bf* w, bf* q, bf* k, bf* v, bf* a, bf* b,
    bf* mask, bf* dy, float* s, float* sa, float* dht, float* dh0, bf* dw, bf* dq, bf* dk, bf* dv, bf* da, bf* db);
void cuda_forward_inference_with_mask(int B, int T, int H, bf* w, bf* q, bf* k, bf* v, bf* a, bf* b,
    bf* y, float* s, float* h0, bf* mask);

void forward(
    torch::Tensor &w, torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
...
void forward_with_mask(
    ...
    cuda_forward_with_mask(B, T, H,
        (bf*)w.data_ptr(), (bf*)q.data_ptr(), (bf*)k.data_ptr(), (bf*)v.data_ptr(),
        (bf*)a.data_ptr(), (bf*)b.data_ptr(), (bf*)y.data_ptr(),
        (float*)s.data_ptr(), (float*)sa.data_ptr(), (float*)h0.data_ptr(), (bf*)mask.data_ptr());
}

void backward_with_mask(
    torch::Tensor &w, torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
    torch::Tensor &a, torch::Tensor &b, torch::Tensor &mask,
    torch::Tensor &dy, torch::Tensor &s, torch::Tensor &sa, torch::Tensor &dht, torch::Tensor &dh0,
    torch::Tensor &dw, torch::Tensor &dq, torch::Tensor &dk, torch::Tensor &dv, torch::Tensor &da, torch::Tensor &db
) {
    int B = w.sizes()[0], T = w.sizes()[1], H = w.sizes()[2];
    cuda_backward_with_mask(B, T, H,
        (bf*)w.data_ptr(), (bf*)q.data_ptr(), (bf*)k.data_ptr(), (bf*)v.data_ptr(),
        (bf*)a.data_ptr(), (bf*)b.data_ptr(), (bf*)mask.data_ptr(), (bf*)dy.data_ptr(),
        (float*)s.data_ptr(), (float*)sa.data_ptr(), (float*)dht.data_ptr(), (float*)dh0.data_ptr(),
        (bf*)dw.data_ptr(), (bf*)dq.data_ptr(), (bf*)dk.data_ptr(),
        (bf*)dv.data_ptr(), (bf*)da.data_ptr(), (bf*)db.data_ptr());
}

void forward_inference_with_mask(
    torch::Tensor &w, torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
    torch::Tensor &a, torch::Tensor &b, torch::Tensor &mask,
    ...
    cuda_forward_inference_with_mask(B, T, H,
        (bf*)w.data_ptr(), (bf*)q.data_ptr(), (bf*)k.data_ptr(), (bf*)v.data_ptr(),
        (bf*)a.data_ptr(), (bf*)b.data_ptr(), (bf*)y.data_ptr(),
        (float*)s.data_ptr(), (float*)h0.data_ptr(), (bf*)mask.data_ptr());
}

TORCH_LIBRARY(TABICL_RWKV7_OP_NAMESPACE, m) {
...
TORCH_LIBRARY_IMPL(TABICL_RWKV7_OP_NAMESPACE, CUDA, m) {
